...
5. Optional: point to `Bloomreach saved segment proof` when Story 139 metadata is configured.
   - "We also proved the mutation path once through operator-run Cursor MCP: a saved segment exists in Bloomreach, but this page only displays the recorded object metadata. It does not create or update Bloomreach on page load."
...
9. Point to `Sandbox proof` inside the appendix.
...
10. Point to `Bloomreach saved segment proof` inside the appendix when configured.
   - "The saved segment proof is also bounded: it proves a real sandbox object can be created through Cursor MCP, but it does not send a campaign, trigger a flow, or become revenue truth."

11. Point to the cross-system demo map.
...
12. Point to `Live Loomi schema proof`.
...
13. Point to `Reviewable recovery brief`.
...
15. Point to `Bloomreach-ready segment recipe`.
...
17. Point to the diagnosis and prepared action.
...
18. Point to the evidence boundary.
...
19. Open Reports, then `Why this revenue counted`.
...
- Saved-segment proof layer: optional recorded metadata from one Cursor MCP-created Bloomreach saved segment, displayed without page-load mutation.
...
- "When configured, the saved-segment proof shows the mutation path worked once through Cursor MCP, while the app remains a no-send, no-page-load-mutation review surface."
...
4. Optional Story 139 mutation proof: ask Cursor to create one safe saved segment, return the saved segment name/ID, and configure only those sanitized metadata fields.
5. If Cursor reports that no saved segment write/create tools exist, say that plainly: the MCP surface proved the schema and project context but did not expose mutation tools, so no object ID was fabricated.
