...
def test_migrations_upgrade_and_downgrade():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            assert "billing_account_id" in creator_columns
            assert "billing_provider_correlation_id" in creator_columns
            assert "run_generator_type" in experiment_run_columns
            assert "run_model_name" in experiment_run_columns
            assert "run_config_version" in experiment_run_columns
            assert "claim_generator_type" in claim_snapshot_columns
            assert "claim_model_name" in claim_snapshot_columns
            assert "claim_config_version" in claim_snapshot_columns
            assert "payment_provider" in invoice_columns
            assert "provider_account_id" in invoice_columns
            assert "provider_invoice_id" in invoice_columns
            assert "payment_provider" in payment_event_columns
...
def test_creator_operator_experiment_draft_run_cards_table_has_expected_columns_fk_indexes_and_unique_constraints():
    ...
    with engine.connect() as conn:
        ...
        assert any(
            constraint["name"] == "uq_creator_experiment_run_cards_run_card_id"
            and constraint["column_names"] == ["run_id", "card_id"]
            for constraint in unique_constraints
        )
