    ...
    def send_magic_link(self, message: MagicLinkEmailMessage) -> None:
        try:
            self._send_email(
                ...
                html_body=_magic_link_email_html_body(message),
            )
...
def _magic_link_email_html_body(message: MagicLinkEmailMessage) -> str:
    escaped_url = escape(message.magic_link_url, quote=True)
    return (
        "<html><body>"
        "<p>Use this secure sign-in link:</p>"
        f'<p><a href="{escaped_url}">Sign in securely</a></p>'
        "<p>If the button does not open, copy and paste this link into your browser:</p>"
        f'<p><a href="{escaped_url}">{escaped_url}</a></p>'
        f"<p>This link expires in {message.expires_in_minutes} minutes and can only be used once.</p>"
