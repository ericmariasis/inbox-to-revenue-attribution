...
def test_paypal_connect_callback_browser_response_shows_payments_receivable_failure_copy_from_email_override():
    inserted = _insert_creator_user(email=f"paypal_browser_receivable_override_{uuid.uuid4().hex}@example.com")
    access_token = _access_token(
        user_id=inserted["user_id"],
        creator_id=inserted["creator_id"],
        email=inserted["email"],
    ...
    provider = _StubPayPalProvider(
        seller_status=PayPalSellerStatus(
            merchant_id="merchant_should_not_be_used",
            tracking_id="placeholder",
            payments_receivable=True,
            primary_email_confirmed=True,
        )
    )
    settings = _paypal_operator_only_settings(inserted["email"], environment="sandbox").model_copy(
        update={
            "paypal_mock_connect_payments_receivable_false_emails": inserted["email"],
        }
    )
            ...
            with TestClient(app) as client:
                ...
                callback_response = client.get(
                    ...
                    params={
                        "state": state,
                        "merchantId": tracking_id,
                        "merchantIdInPayPal": "merchant_pp6_receivable_override",
                        "permissionsGranted": "true",
                        "consentStatus": "true",
                    },
                    headers=HTML_ACCEPT_HEADERS,
...
def test_paypal_connect_callback_browser_response_shows_primary_email_failure_copy_from_email_override():
    inserted = _insert_creator_user(email=f"paypal_browser_email_override_{uuid.uuid4().hex}@example.com")
    access_token = _access_token(
        user_id=inserted["user_id"],
        creator_id=inserted["creator_id"],
        email=inserted["email"],
    ...
    settings = _paypal_operator_only_settings(inserted["email"], environment="sandbox").model_copy(
        update={
            "paypal_mock_connect_primary_email_false_emails": inserted["email"],
        }
    )
            ...
            with TestClient(app) as client:
                ...
                callback_response = client.get(
                    ...
                    params={
                        "state": state,
                        "merchantId": tracking_id,
                        "merchantIdInPayPal": "merchant_pp6_email_override",
                        "permissionsGranted": "true",
                        "consentStatus": "true",
                    },
                    headers=HTML_ACCEPT_HEADERS,
