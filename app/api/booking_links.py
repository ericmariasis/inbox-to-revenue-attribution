...
def list_booking_link_responses_for_creator(
    *,
    creator_id: UUID,
    db: Session,
) -> list[BookingLinkResponse]:
    booking_links = list(
        db.execute(_creator_scoped_booking_links_query(creator_id=creator_id)).scalars()
    )
...
def create_booking_link_response_for_creator(
    *,
    creator_id: UUID,
...
def list_booking_links(
    ...
    return list_booking_link_responses_for_creator(
...
def create_booking_link(
    ...
    return create_booking_link_response_for_creator(
        creator_id=current_user.creator_id,
        payload=payload,
        db=db,
    )
